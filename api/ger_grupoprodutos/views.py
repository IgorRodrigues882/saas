...
from .seriealizers import GerGrupoProdutosSerializer


class GerGrupoProdutosViewSet(viewsets.ModelViewSet):
    """
    API endpoint that allows ger_grupoprodutos to be viewed, edited or created.
    """

    queryset = ger_grupoprodutos.objects.all()
    serializer_class = GerGrupoProdutosSerializer
    permission_classes = [permissions.IsAuthenticated]

     # Return data for edit in transportadora-list
    def retrieve(self, request, pk=None):
        try:
            condicao = ger_grupoprodutos.objects.get(pk=pk)
        except ger_grupoprodutos.DoesNotExist:
