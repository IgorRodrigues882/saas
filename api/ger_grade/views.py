...
from .seriealizers import GerGradeSerializer


class GerGradeViewSet(viewsets.ModelViewSet):
    """
    API endpoint that allows ger_grade to be viewed, edited or created.
    """

    queryset = ger_grade.objects.all()
    serializer_class = GerGradeSerializer
    permission_classes = [permissions.IsAuthenticated]

    # Return data for edit in transportadora-list
    def retrieve(self, request, pk=None):
        try:
            condicao = ger_grade.objects.get(pk=pk)
        except ger_grade.DoesNotExist:
