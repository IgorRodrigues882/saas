      ...
      Dropzone.options.csvImportarquivo = {
        ...
        function gera_tabela(index){
          ...
          for(let i = 0; i < index.length; i++){
            ...
            let badge = ''
            let exp;
            if(index[i].status == 'APROVADO'){
                badge = 'badge badge-success'
            }
            else if(index[i].status == 'PENDENTE'){
                badge = 'badge badge-warning'
                exp = 1
            }
            else if(index[i].status == 'REJEITADO'){
                badge = 'badge badge-danger'
                exp = 1
            }
            else if(index[i].status == 'CANCELADO'){
            ...
            <td>
              <h6>${formatarData(index[i].created_at)}</h6>
            </td>
            <td>
              <h6><span class="${badge} text-white">${tipo}</span></h6>
            </td>
            <td>
              <h6>${index[i].data_tx ? dataformat2(index[i].data_tx) : '-'}</h6>
            </td>
            <td>
              <h6>${index[i].data_vencimento ? dataformat2(index[i].data_vencimento) : '-'}<span class='badge badge-danger ${index[i].expirou == 'S' ? 'd-block' : 'd-none'}' style='font-size: 0.8em; color: white;'>Expirou</span></h6>
            </td>
            <td>
