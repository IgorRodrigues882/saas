...
var options = {
    labels: [termo, sem_venda_termo],
    series: [parseInt(vendas), parseInt(sem_compra)],
    chart: {
      type: 'donut',
      height: 320 ,
    },
  ...
  for(let i = 0; i < index.length; i++){
      ...
      rowsHTML += `
          <tr>
              ...
              <td>
                 <a class="btn btn-secondary" onclick="detalhes_pagamento('${index[i].id}')" data-bs-toggle="modal" data-bs-target="#modal-pix-detalhes">Ver detalhes</a>
              </td>
          </tr>
      `;
  }
...
function detalhes_pagamento(id){
  ...
  .then(data=>{
    ...
    if(data.id){
      ...
      $("#txid").val(data.txid).attr('disabled', true)
...
$("#salvar_comprovante").on('click', function(){
  ...
  $("#export_historico").on('click', function(){
    ...
    fetch(`/pt/api/pix_transaction/gerar_excel_historico/`, {  // Substitua pela URL da sua API
        ...
        a.href = url;
        a.download = `historico_doações_${entidade_nome}.xlsx`;
