...
var candidatos = (function(){
    ...
    function carrega_itens(page){
        ...
        const filtros = {
            status: $("#status").val(),
            "search-candidatos": $("#search_candidatos").val().trim(),
            vaga: $("#vaga").val(),
            unidade: $("#unidade").val()
        };

        fetch(`api/candidatos/filtragem_candidates/?page=${page}`, {
            method: 'POST',
            headers: {
    ...
    function gera_tabela(index){
        ...
        if(index.length === 0) return '<tr><td colspan="10" style="text-align: center;">Nenhum dado encontrado</td></tr>';
        ...
        return index.map(item => {
            ...
            <tr>
                ...
                <td><h6 class="descricao" title="${item.candidate}">${item.candidate}</h6></td>
                <td><h6 class="descricao" title="${item.status}" style="color: ${item.corkankan};">${item.status}</h6></td>
                <td><h6 class="descricao" title="${item.cpf}">${formatarCPFouCNPJ(item.cpf)}</h6></td>
                <td><h6 class="descricao" title="${item.phone}">${item.phone}</h6></td>
                <td><h6 class="descricao" title="${item.dtcadastro}">${formatarData(item.dtcadastro)}</h6></td>
                <td><h6 class="descricao" title="${item.dtiniciopreenchimento}">${formatarData(item.dtiniciopreenchimento)}</h6></td>
                <td><h6 class="descricao" title="${item.dtultimopreenchimento}">${formatarData(item.dtultimopreenchimento)}</h6></td>
                <td><h6>${formatarData(item.dtenvioflow)}</h6></td>
                <td>
                    ...
                    <div class="dropdown-menu">
                        <a class="dropdown-item" href="javascript:void(0)" 
                            onclick="candidatos.deleteCadidate(${item.candidate_id})">
                            Excluir
                        </a>
                        <a class="dropdown-item" href="javascript:void(0)" 
                            data-bs-toggle="modal" data-bs-target="#addcandidato" 
                            onclick="candidatos.editarCandidate(${item.candidate_id})">
                            Editar
                        </a>
                    </div>
                </td>
            ...
            if (result.isConfirmed) {
                ...
                .then(response => {
                    if (response.status === 204) {
                        Swal.fire("Sucesso", "Candidato excluído!", "success");
                        carrega_itens(1);
                    } else {
...
function formatDateForInput(dateString) {
    ...
    function editarCandidate(id){
        editar_candidate = true;
        id_candidate = id;
        ...
        fetch(`api/candidatos/${id}/`)
            ...
            .then(data => {
                ...
                $("#cpf").val(data.cpf);
                $("#unidade_id").val(data.unidade_id).trigger('change');
                $("#job_id").val(data.job_id).trigger('change');
                $("#recrutador_id").val(data.recrutador_id).trigger('change');
                $("#dtiniciopreenchimento").val(formatDateForInput(data.dtiniciopreenchimento));
                $("#dtenvioflow").val(formatDateForInput(data.dtenvioflow));
                $("#tokencandidate").val(data.tokencandidate);
                $("#notasinternas").val(data.notasinternas);
            })
    ...
    function init(){
        ...
        (function ($) {
            const bloodhound = new Bloodhound({
              ...
              remote: {
                url: `api/candidatos/busca/?query=%QUERY`,
                wildcard: "%QUERY"
              }
            });
        ...
        if (exportBtn && exportFormatSelect && exportModalElement) {
            ...
            exportBtn.addEventListener('click', async () => {
                ...
                const fieldsToExport = [
                    'candidate_id', 'candidate', 'status_id__status_description', 'cpf', 'phone', 'email',
                    'job_id__title', 'unidade_id__name', 'dtcadastro', 'dtiniciopreenchimento',
                    'dtultimopreenchimento', 'dtenvioflow'
                ];
                const fieldLabels = {
                    'candidate_id': 'ID',
                    'candidate': 'Nome Completo',
                    'status_id__status_description': 'Status',
                    'cpf': 'CPF',
                    'phone': 'Telefone',
                    'email': 'E-mail',
                    'job_id__title': 'Vaga Associada',
                    'unidade_id__name': 'Unidade',
                    'dtcadastro': 'Data Cadastro',
                    'dtiniciopreenchimento': 'Início Preenchimento',
                    'dtultimopreenchimento': 'Último Preenchimento',
                    'dtenvioflow': 'Data Envio Flow'
                };
                ...
                const currentFilters = {
                    status: $("#status").val(),
                    "search-candidatos": $("#search_candidatos").val().trim(),
                    vaga: $("#vaga").val(), // Assumindo que o ID do filtro de vaga é 'vaga'
                    unidade: $("#unidade").val() // Assumindo que o ID do filtro de unidade é 'unidade'
                };
                ...
                const payload = {
                    model_name: 'recrutamento.Candidates', // Corresponde ao modelo Django
                ...
                try {
                    ...
                    if (response.ok) {
                        ...
                        a.download = `candidatos_export_${timestamp}.${exportFormat}`;
    ...
    return {
        init: init,
        deleteCadidate: deleteCadidate,
        editarCandidate: editarCandidate,
        newCandidato: () => {
            editar_candidate = false;
            id_candidate = 0;
            $("#candidate").val('');
            ...
            $("#email").val('');
            $("#cpf").val('');
            $("#unidade_id").val('').trigger('change');
            $("#job_id").val('').trigger('change');
            $("#recrutador_id").val('').trigger('change');
            $("#dtiniciopreenchimento").val('');
            $("#dtenvioflow").val('');
            $("#tokencandidate").val('');
            $("#notasinternas").val('');
        }
