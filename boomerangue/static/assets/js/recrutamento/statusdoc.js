...
var statusdoc = (function(){
  ...
  function carrega_itens(page){
      let tbody = document.getElementById('tbody-statusdoc');
      let loader = document.getElementById('loader-statusdoc');
      tbody.innerHTML = '';
      loader.style.display = 'block';
      
      fetch(`api/statusdoc/filtragem_statusdoc/?page=${page}`, {
          method: 'POST',
          headers: {
                  ...
                  <td>
                      <div class="media">
                          <div class="square-box me-2">
                              <img class="img-fluid b-r-5" src="/static/assets/images/job-search/1.jpg" alt="">
                          </div>
                          <div class="media-body ps-2">
                              <div class="avatar-details">
                                  <a href="javascript:void(0)">
                                      <h6>${index[i].document_status}</h6>
                                  </a>
                                  <span>${index[i].document_status_id}</span>
                              </div>
                          </div>
                      </div>
                  </td>
                  <td>
                      ...
                      <div class="dropdown-menu">
                          <a class="dropdown-item" href="javascript:void(0)" 
                              onclick="statusdoc.deletestatusdoc(${index[i].document_status_id})">
                              Excluir
                          </a>
                          <a class="dropdown-item" href="javascript:void(0)" 
                              data-bs-toggle="modal" data-bs-target="#addstatusdoc" 
                              onclick="statusdoc.editarstatusdoc(${index[i].document_status_id})">
                              Editar
                          </a>
                      </div>
                  </td>
  ...
  function updatePagination(count) {
      ...
      const pagination = document.getElementById('pagination-statusdoc');
      ...
      for (let i = 1; i <= totalPages; i++) {
          ...
          li.appendChild(a);
          pagination.insertBefore(li, document.getElementById('next-page-statusdoc'));
      }
      
      document.getElementById('previous-page-statusdoc').classList.toggle('disabled', currentPage === 1);
      document.getElementById('next-page-statusdoc').classList.toggle('disabled', currentPage === totalPages);
  }
  ...
  function init(){
    ...
    //           prefetch: "",
    //           remote: {
    //               url: `api/statusdoc/busca/?query=%QUERY`,
    //               wildcard: "%QUERY",
    ...
    //   })(jQuery);
      ...
      document.getElementById('previous-page-statusdoc').addEventListener('click', () => {
          if (currentPage > 1) loadPage(currentPage - 1);
      });

      document.getElementById('next-page-statusdoc').addEventListener('click', () => {
          if (currentPage < totalPages) loadPage(currentPage + 1);
      });

      // Evento de salvar
      $("#salvar_statusdoc").on("click", function(){
          Swal.fire({
              title: 'Aguarde',
          ...
          const url = editar_vaga ? `api/statusdoc/${id_statusdoc}/` : "api/statusdoc/";
          const method = editar_vaga ? 'PATCH' : 'POST';
          const document_status = $("#document_status").val();
          const document_status_short = $("#document_status_short").val();

          if (!document_status || !document_status_short) {
              Swal.fire({
          ...
          formData.append('document_status_short', document_status_short);
      ...
      $("#export_btn_statusdoc").on('click', function(){
        Swal.fire({
            title: 'Aguarde',
        ...
        const exportFormat = $("#export_format_statusdoc").val()?.toLowerCase().trim() || 'csv';
        const modelName = 'recrutamento.DocumentStatus';

        // Define os campos TÉCNICOS que você quer exportar
        const fieldsToExport = [
            'document_status_id',
            'document_status',
            'document_status_short',
            'dtcadastro',
        ];
        ...
        const fieldLabels = {
            'document_status_id': 'ID', // Label para ID (não está na tabela, mas pode ser útil)
            'document_status': 'Status',
            'document_status_short': 'Descrição curta', // Label para o campo corrigido
  ...
  function editarstatusdoc(id){
      ...
      fetch(`api/statusdoc/${id}/`)
          ...
          .then(data => {
              ...
              $("#document_status_short").val(data.document_status_short);
          })
  ...
  function newVaga(){
      editar_vaga = false;
      id_statusdoc = 0;
      $("#document_status, #document_status_short").val('');
  }

  return {
      init: init,
      deletestatusdoc: deletestatusdoc,
      editarstatusdoc: editarstatusdoc,
      newVaga: newVaga,
