...
var tabela_produtos = function(){
    ...
    var init = function(){
        ...
        function ajusta_produtos(){
          let tbody = document.getElementById('tb_produtos')
          tbody.innerHTML = ''
          fetch('api/bmm_boomerangueitens/produtos_vendidos/')
          .then((res)=>res.json())
          .then(data=>{
            console.log(data)
            if(data.length > 0){
                tbody.innerHTML = gera_tabela_produtos(data)
            }
            else{
        ...
        function gera_tabela_produtos(index){
          ...
          for(let i = 0; i < index.length; i++){
            ...
            <tr>
              <td>
                <div class="media">
                  <div class="table-box-img bg-light-primary">
                  <img class="img-fluid"
                  src="${index[i].foto}" alt="">
                  </div>
                  <div class="media-body">
                    <div class="avatar-details"><a href="#">
                        <h6>${index[i].nome_do_produto}</h6>
                      </a><span> </span></div>
                  </div>
                </div>
              </td>
              <td>
                <h6>Vendas: ${index[i].total_vendido}</h6><span>${formatarMoedaBrasil(index[i].valor)}</span>
              </td>
              <td><span>Em todas Campanhas</span></td>
            </tr>
            `
          }
