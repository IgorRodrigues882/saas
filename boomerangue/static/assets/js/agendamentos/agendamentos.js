...
var pix = function(){
      ...
      Dropzone.options.csvImportarquivo = {
        ...
        function gera_tabela(index){
          ...
          for (let i = 0; i < index.length; i++) {
            let tipo = index[i].bm_status;
            let badge = '';
            let text = ''

            // Define a classe badge com base no status
            if (tipo === 'T') {
              ...
              text = 'Confirmado depois do horário'
            }
            else{
            ...
            rowsHTML += `<tr>
              ...
              <td>
                <h6>${index[i].data_consulta} às ${index[i].hora_consulta}</h6>
              </td>
              <td>
                <h6><span class="${badge} text-white">${text}</span></h6>
              </td>`;

            // Preencher os campos dinâmicos
            // Obter os campos dinâmicos do cabeçalho
            const campos = document.querySelectorAll('#campo_dinamico');

            // Adiciona as células para cada campo dinâmico antes do botão
            for (let campo of campos) {
              const campoNome = campo.classList[0]; // Pega o nome do campo a partir da classe
              const valorCampo = index[i][campoNome] || ''; // Obtém o valor do campo dinâmico
              rowsHTML += `<td>${valorCampo}</td>`; // Adiciona a célula correspondente
            }

            // Adiciona a coluna do botão depois dos campos dinâmicos
            rowsHTML += `<td>
    ...
    document.getElementById('next-page').addEventListener('click', () => {
      ...
      function carrega_itens(page){
        ...
        fetch(`api/bmm_boomerangue/filtragem/?page=${page}`, {  // Substitua pela URL da sua API
          method: 'POST',
          headers: {
    ...
    $("#export_btn").on('click', function(){
      ...
      fetch(`api/pix_transaction/gerar_excel_historico/`, {  // Substitua pela URL da sua API
          ...
          a.href = url;
          a.download = `tabela_pix_gerados_${dataformat2(date)}.xlsx`;
    ...
    $('#baixar_integrador').click(function(e) {
      e.preventDefault();
      
      var $link = $(this);
      var originalText = $link.text();
      $link.text('Preparando download...');
      
      $.ajax({
          url: 'api/grupoAgendamentos/download_integrador/',
          method: 'GET',
          xhrFields: {
              responseType: 'blob'
          },
          success: function(response) {
              ...
              a.style.display = 'none';
              a.href = url;
              a.download = 'integrador_package.zip';
              document.body.appendChild(a);
              a.click();
